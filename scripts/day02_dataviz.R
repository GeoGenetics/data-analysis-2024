...
# for the remainder of this tutorial we will use a subset of a few countries

countries <- c("Israel", "Japan", "Denmark", "South Africa", "India")
...
# Coordinate systems determine the positioning of the data onto the graphic.
# The default coordinate system in ggplot2 is the Cartesian coordinate system,
# but other ones are also available.

# Try using the `coord_` syntax to zoom into parts of the plot
# or change the coordinate system entirely

# basic scatterplot with explicit coordinate system
ggplot(covid_data_subset) +
    geom_point(mapping = aes(
        x = date,
        y = total_vaccinations
...
ggplot(covid_data_subset) +
    ...
    coord_cartesian(
        xlim = c(ymd(20210101), ymd(20210601)), ## the `ymd()` function helps to parse date-time variables
        ylim = c(0, 1e6)
    )

# other coordinate system (polar)
ggplot(covid_data_subset) +
    geom_point(mapping = aes(
        x = date,
        y = total_vaccinations
...
ggplot(covid_data_subset) +
    ...
    facet_grid(location ~ .) ## `location` used for rows, no variable for columns
