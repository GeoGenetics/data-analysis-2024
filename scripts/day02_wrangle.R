...
# This COVID19 epidemiological dataset contains 399,281 rows (observations)
# of 67 columns (variables)
...
# by variable name
select(covid_data, location)
select(covid_data, total_cases, location)

# by variable column index
select(covid_data, 1, 5)

# by variable column index and name
select(covid_data, 1, location)
...
select(covid_data, location, everything()) ## `everything()` selects all remaining columns
...
filter(covid_data, location == "Denmark")
...
filter(covid_data, location == "Denmark", date == "2020-03-01")
filter(covid_data, location == "Denmark" | date == "2020-03-01")
...
distinct(covid_data, location)
distinct(covid_data, location, continent)
...
# filter our data for location "Sweden"
...
covid_data %>%
  ...
  select(location, date, new_cases, new_cases_percent_pop)
...
## Q:
# For each continent and location:
...
# Which country had the highest average new case rates?
...
# per location for this part of the tutorial.

un_pop <- read_csv("https://github.com/GeoGenetics/data-analysis-2024/raw/main/data/un_pop.csv")
un_pop
...
# Our example table encodes a single variable (the population for a location)
...
# with the variable names to be reshaped as a rgument.
...
# The following code produces a wide format table with one line per location,
# and a separate variable for each year.

un_pop %>%
  select(location, year, pop_65_plus) %>%
  pivot_wider(
    names_from = "year",
...
un_pop %>%
  select(location, year, pop_6_11, pop_12_24, pop_25_64) %>%
  pivot_longer(-location:-year) %>%
...
# calculate test positivity rate, averaged per month for each year and location,
